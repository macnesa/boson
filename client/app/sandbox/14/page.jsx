...
// ============================
// BOSON STATIC COLOR SYSTEM
// ============================
const USE_STATIC_PALETTE = true

const ColorPalette = {
  eventHorizon: new THREE.Color('#090909'),   // Background
  cosmicLight: new THREE.Color('#F3F4F5'),    // Ambient fog
  supernovaBurst: new THREE.Color('#F8A13D'), // Swarm outer energy
  quantumFlame: new THREE.Color('#BC4A26'),   // Inner shell
  bigBangCore: new THREE.Color('#892130'),    // Core
}
...
function DynamicLighting() {
  ...
  useFrame((_, delta) => {
    ...
    if (USE_STATIC_PALETTE) {
      GlobalUniforms.uHue.value = 0.0
      GlobalUniforms.uHueCore.value = 0.0
      GlobalUniforms.uHueShell.value = 0.0
      GlobalUniforms.uHueSwarm.value = 0.0
    } else {
    ...
    if (key.current && rim.current && fill.current) {
      key.current.color = ColorPalette.supernovaBurst
      rim.current.color = ColorPalette.quantumFlame
      fill.current.color = ColorPalette.bigBangCore
  
      const breathe = 0.5 + Math.sin(t * 0.8) * 0.3
      key.current.intensity = 1.0 + breathe * 0.45 * (0.6 + beat * 0.8)
      rim.current.intensity = 0.9 + breathe * 0.4
      fill.current.intensity = 0.25 + breathe * 0.2
    }
  })
  ...
  return (
    <>
      ...
      <pointLight ref={fill} intensity={0.45} distance={8} decay={2} position={[0, -2, 3]} />
      <ambientLight intensity={0.05} color={ColorPalette.eventHorizon} />
    </>
  )
}
// ---------------------------
      ...
      <shaderMaterial
        ...
        fragmentShader={`
          ...
          void main() {
            ...
            // vec3 col = vec3(0.54, 0.13, 0.19); // Big Bang Core (#892130)
            vec3 col = vec3(0.74, 0.29, 0.15); // Quantum Flame (#BC4A26)
...
function useOrbitGPU_VelPos(renderer, size = 512, baseRadius = 8.0, GlobalUniforms, GlobalPhase) {
  ...
  useEffect(() => {
    ...
    // --- velocity fragment (magnetic filament + gravitational collapse/explosion + curl noise) ---
    const velFrag = `
    precision highp float;
    varying vec2 vUv;
    ...
    // additional controls
    uniform float uFieldStrength;   // magnetic filament strength
    uniform float uGravStrength;    // gravity pull strength
    uniform float uCollapseAmp;     // amplitude for collapse/expansion oscillation
    uniform float uExplosionBoost;  // short outward impulse multiplier
    ...
    // magnetic-like filament field (smooth, directed)
    vec3 magneticField(vec3 p, float t) {
      // field wraps around axis and has gentle Z perturb
      vec3 f = vec3(-p.y, p.x, 0.2 * sin(p.z * 0.35 + t * 0.6));
      return normalize(f + 0.0001);
    }

    // radial wave used for outward explosion ripple
    float radialWave(vec3 p, float t) {
      float d = length(p);
      return sin(t * 2.0 - d * 2.2);
    }
    ...
    void main() {
      ...
      // --- parametric orbit target (kept as gentle attractor) ---
      ...
      // --- spring to target (keeps orbit structure) ---
      vec3 toTarget = target - pos;
      float dist = length(toTarget);
      vec3 spring = (dist > 0.0001) ? normalize(toTarget) * (dist * 0.018) : vec3(0.0);

      // --- curl noise for soft turbulence ---
      vec3 curl = curlNoise(pos * 0.25 + uTime * 0.15) * (uTurbulence * 0.6);

      // --- magnetic filament field (gives directional flow) ---
      vec3 field = magneticField(pos * 0.6, uTime) * uFieldStrength * (0.5 + 0.5 * layer);

      // --- gravitational collapse/expansion oscillator ---
      // collapseOsc in [-1..1] controls pull/expel
      float collapseOsc = sin(uTime * 0.5 + uPhaseSwarm);
      float collapseFactor = 1.0 + uCollapseAmp * collapseOsc; // 1 ± amp
      vec3 gravDir = -normalize(pos + vec3(0.0001));
      vec3 grav = gravDir * uGravStrength * (0.6 + 0.4 * (0.5 + 0.5 * collapseOsc));

      // --- explosion ripple impulse (short bursts outward) ---
      float wave = radialWave(pos, uTime + s * 0.2);
      float explosion = smoothstep(0.25, 0.55, (wave * 0.5 + 0.5)) * uExplosionBoost * (uBeat + 0.05);
      vec3 explosionForce = normalize(pos) * explosion;

      // --- repulsion to avoid clumping (soft) ---
      vec3 repel = normalize(pos) * -0.06;

      // --- combine accelerations ---
      vec3 acc = spring + curl + field + grav + repel + explosionForce;

      // --- integrate velocity with damping and sim speed ---
    ...
    // --- position fragment integrates vel into pos (slower, heavy feel) ---
    const posFrag = `
    precision highp float;
    varying vec2 vUv;
    ...
    void main() {
      ...
      // heavier inertia: scale step factor down for cinematic weight
      vec3 newPos = pos + vel * uDelta * 18.0 * uSimSpeed;
    ...
    const velMat = new THREE.ShaderMaterial({
      ...
      uniforms: {
        ...
        uSimSpeed: { value: 0.25 },  // slower sim speed for cinematic weight
        uDelta: { value: 1 / 60 },

        // new controls for magnetic + collapse behavior
        uFieldStrength: { value: 0.085 },  // filament pull
        uGravStrength: { value: 0.18 },    // central gravity
        uCollapseAmp: { value: 0.65 },     // collapse/expansion amplitude
        uExplosionBoost: { value: 0.9 },  // burst intensity on beats
      },
      depthTest: false,
      depthWrite: false,
  ...
  useFrame((state, delta) => {
    ...
    // ... lanjut dari baris terakhir yang lo kirim
    ...
    velMat.uniforms.uCollapse.value = GlobalUniforms.uCollapse ? GlobalUniforms.uCollapse.value : 0
    velMat.uniforms.uPhaseSwarm.value = phase
    velMat.uniforms.uDelta.value = delta

    // dynamic tuning: make explosion boost react to beat transiently
    const beatImpulse = Math.pow(beat, 2.4)
    velMat.uniforms.uExplosionBoost.value = 0.9 + 2.0 * beatImpulse

    // optionally modulate field & gravity slightly with phase for organic feel
    velMat.uniforms.uFieldStrength.value = 0.085 + 0.04 * Math.sin(GlobalPhase.shell * 1.2)
    velMat.uniforms.uGravStrength.value = 0.18 + 0.06 * (0.5 + 0.5 * Math.sin(GlobalPhase.core * 0.9))

    // --- velocity pass (write into velWrite) ---
    ...
    // publish updated textures back to sim
      ...
      <shaderMaterial
        ...
        fragmentShader={`
          ...
          void main() {
            ...
            vec3 col = vec3(0.95, 0.96, 0.97); // Cosmic Light (#F3F4F5)
            // vec3 col = vec3(0.97, 0.63, 0.24); // Supernova Burst (#F8A13D)
...
function Scene() {
  return (
    <>
      ...
      <fog attach="fog" args={[ColorPalette.eventHorizon, 3.5, 14]} />
      <color attach="background" args={[ColorPalette.eventHorizon]} />

      <Suspense fallback={null}>
        <Environment preset="studio" background={false} blur={0.2} />
      </Suspense>

      <DynamicLighting />
      <VolumetricShell radius={4.2} />
      <OrbitingSwarmBehavior texRes={300} baseRadius={8.0} />

      <AuroraSphere />

      <EffectComposer disableNormalPass>
        <Bloom intensity={0.65} luminanceThreshold={0.28} />
      ...
      <ScreenSpaceVolumetricPass
        ...
        lightColor={ColorPalette.supernovaBurst.clone()}
      />
    </>
  )
}
